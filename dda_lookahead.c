 ...
 */
void dda_join_moves_org(DDA *prev, DDA *current) {
...
void dda_join_moves(DDA *prev, DDA *current) {
	...
	uint32_t prev_F, prev_F_in_steps, prev_F_start_in_steps, prev_F_end_in_steps;
	uint32_t prev_rampup, prev_rampdown, prev_total_steps;
	uint32_t crossF, crossF_in_steps, max_crossF_in_steps;
	uint8_t prev_id;
	// Similarly, we only want to modify the current move if we have the results of the calculations;
	// until then, we do not want to touch the current move settings.
	// Note: we assume 'current' will not be dispatched while this function runs, so we do not to
	...
	uint32_t this_F, this_F_in_steps, this_F_start_in_steps, this_rampup, this_rampdown, 
		this_total_steps, current_F_end_in_steps;
	uint8_t this_id;
	static uint32_t la_cnt = 0;     // Counter: how many moves did we join?
#ifdef LOOKAHEAD_DEBUG
	static uint32_t moveno = 0;     // Debug counter to number the moves - helps while debugging
	...
	if (prev->live == 0) {
		...
		ATOMIC_START
			...
			this_F = current->endpoint.F;
			this_total_steps = current->total_steps;
			current_F_end_in_steps = current->end_steps;
		ATOMIC_END
		...
		prev_F_in_steps = ACCELERATE_RAMP_LEN(prev_F);
		this_F_in_steps = ACCELERATE_RAMP_LEN(this_F);
		crossF_in_steps = max_crossF_in_steps = ACCELERATE_RAMP_LEN(crossF);

		// Show the proposed crossing speed - this might get adjusted below
		serprintf(PSTR("Max cross-speed %lu reached in %lu steps\r\n"), crossF, crossF_in_steps);
		serprintf(PSTR("Prev length is %lu steps, current length is %lu steps\r\n"), prev_total_steps, this_total_steps);

		// Compute the maximum speed we can reach for crossing
		crossF_in_steps = MIN(crossF_in_steps, this_total_steps + current_F_end_in_steps);	// TODO: Verify this
		crossF_in_steps = MIN(crossF_in_steps, prev_total_steps + prev_F_start_in_steps);
		
		serprintf(PSTR("Feasable crossing speed is obtained after %lu steps\r\n"), crossF_in_steps);
		...
		// Build ramps for previous move.
		if (crossF_in_steps == prev_F_in_steps) {
			// The crossing speed is the same as the speed obtained after accelerating the entire previous move
			prev_rampup = prev_F_in_steps - prev_F_start_in_steps;
			prev_rampdown = 0;
		}
		else if (crossF_in_steps < prev_F_start_in_steps) {
			// The crossing speed is lower than the crossing speed of the previous move: we need to slow the previous move
			// down to reach the crossing speed between previous and current
			uint32_t extra, limit;

			// Determine the length of the rampdown to reach crossF
			prev_rampup = 0;
			prev_rampdown = prev_F_start_in_steps - crossF_in_steps;
			
			// Before we have to rampdown, it might be possible to speed up at the beginning of the previous move, as long as
			// we are able to slow down to reach crossF in the end.
			extra = (prev_total_steps - prev_rampdown) >> 1;	// calculate 1/2 steps before we need to slow down; spend it accelerating and then breaking
			limit = prev_F_in_steps - prev_F_start_in_steps;	// limit the amount of speedup to prevent going faster than maximum speed prev_F
			extra = MIN(extra, limit);

			// Extend both ramps to go a little faster before slowing down to crossF
			prev_rampup += extra;
			prev_rampdown += extra;
		}
		else {
			// The crossing speed is higher than the crossing speed of the previous move: calculate a ramp to speed the previous move
			// up as much as possible while ending on crossF
			uint32_t extra, limit;

			// Determine the length of the rampup to reach crossF starting from prev_F_start
			prev_rampup = crossF_in_steps - prev_F_start_in_steps;
			prev_rampdown = 0;
			// In case we reach crossF before the move is over, we could speed up more and than slow down to crossF
			extra = (prev_total_steps - prev_rampup) >> 1;
			limit = prev_F_in_steps - crossF_in_steps;
			extra = MIN(extra, limit);
		...
		prev_rampdown = prev_total_steps - prev_rampdown;  // rampdown should denote the step number after which we slow down; NOT the length of the ramp
		prev_F_end_in_steps = crossF_in_steps;             // Save feasible crossing speed

		// Build ramps for current move
		if (crossF_in_steps == this_F_in_steps) {
			// This entire move is used to slow down
			this_rampup = 0;
			this_rampdown = crossF_in_steps - current_F_end_in_steps;	// TODO: Verify this
		}
		else if (crossF_in_steps > current_F_end_in_steps) {
			// Exit speed is lower than entry (crossing) speed: slow down
			this_rampup = 0;
			this_rampdown = crossF_in_steps - current_F_end_in_steps;
			...
			// Exit speed is higher than entry (crossing) speed: speed up
			this_rampup = current_F_end_in_steps - crossF_in_steps;
			this_rampdown = 0;

			uint32_t extra = (this_total_steps - this_rampup) >> 1;
			uint32_t limit = this_F_in_steps - crossF_in_steps;
			extra = MIN(extra, limit);
		...
		serprintf(PSTR("%d: "), prev_id);
		serprintf(PSTR("prev start: %lu, F: %lu, end: %lu, rampup: %lu, rampdown: %lu, crossF: %lu, len: %lu\r\n"), prev_F_start_in_steps, prev_F_in_steps, prev_F_end_in_steps, prev_rampup, prev_total_steps - prev_rampdown, crossF_in_steps, prev_total_steps);
		serprintf(PSTR("%d: "), this_id);
		serprintf(PSTR("this start: %lu, F: %lu, end: %lu, rampup: %lu, rampdown: %lu, len: %lu\r\n"), this_F_start_in_steps, this_F_in_steps, current->end_steps, this_rampup, this_total_steps - this_rampdown, this_total_steps);
